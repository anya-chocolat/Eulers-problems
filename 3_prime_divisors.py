...
for n in range(2, int(digit ** 0.5)):
    ...
    if digit % n == 0:
        ...
        for i in range(2, int(n ** 0.5)):
            if n % i == 0:
                break
        # если число простое, то добавляем его в список простых делителей
        else:
            prime_factors.append(n)
        # проверяем, является ли результат деления простым числом
        if digit // n != 2:
            for i in range(2, int((digit // n) ** 0.5)):
                if digit // n % i == 0:
                    break
            # если результат деления - простое число, то также добавляем его в список
            else:
